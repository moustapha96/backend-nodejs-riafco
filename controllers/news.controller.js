    ...
    try {
        ...
        if (req.files && req.files.galleries) {
            galleries = Array.isArray(req.files.galleries) ?
                req.files.galleries.map((file) => `/news/galleries/${file.filename}`) : [`/news/galleries/${req.files.galleries.filename}`];
        }
...
module.exports.updateNews = async(req, res) => {
    try {
        ...
        const { id } = req.params;
        const { title_fr, title_en, content_fr, content_en, status, publishedAt, validated } = req.body;
        ...
        if (req.files && req.files.galleries) {
            const newGalleries = Array.isArray(req.files.galleries) ?
                req.files.galleries.map((file) => `/news/galleries/${file.filename}`) : [`/news/galleries/${req.files.galleries.filename}`];
            updateData.galleries = [...(existingNews.galleries || []), ...newGalleries];
        }
        ...
        if (updatedNews.status === "PUBLISHED") {
            try {
                await this.sendNewsletterForNews(updatedNews.id);
            } catch (newsletterError) {
                console.error("Failed to send newsletter:", newsletterError);
            }
        }
...
module.exports.deleteNews = async(req, res) => {
    try {
        ...
        if (existingNews.galleries && existingNews.galleries.length > 0) {
            existingNews.galleries.forEach((galleryPath) => {
                const filePath = path.join(__dirname, "..", "uploads", galleryPath);
        ...
        if (existingNews.image) {
            const imagePath = path.join(__dirname, "..", "uploads", existingNews.image);
...
module.exports.updateValidated = async(req, res) => {
    try {
        const { id } = req.params;
...
module.exports.getAllCampaigns = async(req, res) => {
    try {
        ...
        // sécuriser la clé de tri
        const sortableFields = ["createdAt", "updatedAt", "sentAt", "subject", "status", "recipientCount"];
        const _sortBy = sortableFields.includes(sortBy) ? sortBy : "createdAt";
        const _sortOrder = sortOrder.toLowerCase() === "asc" ? "asc" : "desc";
...
module.exports.createCampaign = async(req, res) => {
    try {
        const { subject, content, htmlContent, scheduledAt, newsId } = req.body;

        if (!newsId) {
            return res.status(400).json({ message: "newsId is required", code: "NEWS_ID_REQUIRED" });
        }
...
module.exports.updateCampaign = async(req, res) => {
    try {
        const { id } = req.params;
        const { subject, content, htmlContent, status, scheduledAt, newsId } = req.body;
        ...
        if (updatedCampaign.status === "SENT") {
            if (!updatedCampaign.newsId) {
                return res.status(400).json({
                    message: "Cannot send campaign without a linked newsId",
                    code: "MISSING_NEWS_ID_FOR_SEND",
                });
            }
            await sendNewsletterForNews(updatedCampaign.newsId);
        }
...
module.exports.sendNewsletterForNews = async(newsId) => {
        try {
            ...
            const siteBaseUrl = process.env.BACKEND_URL.replace(/\/$/, "") || "";
            const title = news.title_fr || news.title_en || "RIAFCO News";
            const content = news.content_fr || news.content_en || "";

            const newsletter = {
                    subject: `RIAFCO News: ${title}`,
                    content,
                    htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h1>${title}</h1>
          ${news.image ? `<img src="${siteBaseUrl}${news.image}" alt="${title}" style="max-width: 100%; height: auto;">` : ""}
          <div>${content}</div>
