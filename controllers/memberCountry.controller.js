...
/**
 * @desc Get all member countries (version simplifiée)
 * @route GET /api/member-countries/off
 * @access Public
 */
module.exports.getAllMemberCountriesOff = async (req, res) => {
  try {
    const { sortBy = "name_fr", sortOrder = "asc" } = req.query;
    const validSortOrder = sortOrder === "asc" ? "asc" : "desc";
    ...
    res.status(200).json({ success: true, datas, total });
  } catch (error) {
    console.error("Get all member countries error:", error);
    res.status(500).json({
      success: false,
...
/**
 * @desc Get all member countries (avec pagination, recherche et filtres)
 * @route GET /api/member-countries
 * @access Public
 */
module.exports.getAllMemberCountries = async (req, res) => {
  try {
    const { page = 1, limit = 20, search, status, sortBy = "name_fr", sortOrder = "asc" } = req.query;
    const skip = (page - 1) * limit;
    const take = Number.parseInt(limit);
    ...
    if (search) {
      where.OR = [
        ...
        { pays_fr: { contains: search, mode: "insensitive" } },
        { pays_en: { contains: search, mode: "insensitive" } },
      ];
    }
    if (status) {
      where.status = status;
...
module.exports.createMemberCountry = async (req, res) => {
  try {
    ...
    // Vérifie si le pays existe déjà (par name_fr ou name_en)
    // const existingCountry = await prisma.memberCountry.findFirst({
    //   where: {
    //     OR: [
    ...
    // if (existingCountry) {
    //   return res.status(400).json({
    //     message: "Country name (FR/EN) already exists",
    //     code: "COUNTRY_NAME_EXISTS",
    //   });
    // }
    ...
    const updateData = {
      ...
      pays_fr: pays_fr?.trim(),
      pays_en: pays_en?.trim(),
...
module.exports.updateMemberCountry = async (req, res) => {
  try {
    ...
    // Vérifie si le nouveau name_fr ou name_en existe déjà (hors le pays actuel)
    if (name_fr && name_fr.toLowerCase() !== existingCountry.name_fr?.toLowerCase()) {
      const countryWithSameNameFr = await prisma.memberCountry.findFirst({
        where: {
          name_fr: { equals: name_fr, mode: "insensitive" },
          NOT: { id: existingCountry.id },
        },
      });
      if (countryWithSameNameFr) {
        return res.status(400).json({
          message: "Country name (FR) already exists",
          code: "COUNTRY_NAME_FR_EXISTS",
        });
      }
    }

    if (name_en && name_en.toLowerCase() !== existingCountry.name_en?.toLowerCase()) {
      const countryWithSameNameEn = await prisma.memberCountry.findFirst({
        where: {
          name_en: { equals: name_en, mode: "insensitive" },
          NOT: { id: existingCountry.id },
        },
      });
      if (countryWithSameNameEn) {
        return res.status(400).json({
          message: "Country name (EN) already exists",
          code: "COUNTRY_NAME_EN_EXISTS",
        });
      }
    }
...
module.exports.deleteMemberCountry = async (req, res) => {
  try {
    ...
    await prisma.memberCountry.delete({ where: { id } });
    ...
    await createAuditLog({
      ...
      resourceId: id,
      details: {
        deletedCountry: existingCountry.name_fr,
